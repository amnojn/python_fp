def create_markdown_image(alt_text):
    ...
    def with_url(url):
        ...
        base = enclosed_alt + enclosed_url  # final tanpa title

        def with_title(title=None):
            if title is None:
                return base

            # jika ada title → perlu memasukkan "title"
            quoted_title = f'"{title}"'

            # hapus ')'
            base_without_paren = base[:-1]

            # tambah spasi + quoted title + ')'
            return f"{base_without_paren} {quoted_title})"
